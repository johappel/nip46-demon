 ...
 * @returns {void}
 */
function focusEmbeddedUnlockUi() {
...
/**
 * Entschluesselt einen Keyring-Eintrag fuer den WP-Bridge-Flow ohne UI-Prompt.
 * Nutzt bevorzugt das Session-Passwort, alternativ vorhandenes Unlock-Material (nur bei passendem Salt).
 *
 * @param {object} entry - Keyring-Eintrag mit verschluesseltem Payload.
 * @returns {Promise<string>} Entschluesselter nsec.
 * @throws {Error} Wenn Entschluesselung ohne interaktive Entsperrung nicht moeglich ist.
 */
async function decryptKeyringEntryForWpBridge(entry) {
    if (!entry || !isValidEncryptedPayload(entry.payload)) {
        throw new Error("Gebundener Schlüssel ist ungültig.");
    }

    if (sessionPassword) {
        return decryptNsec(entry.payload, sessionPassword);
    }

    if (
        hasSessionUnlockMaterial() &&
        entry.payload.salt === sessionUnlockMaterial.salt
    ) {
...
/**
 * Erzeugt einen neuen Keyring-Eintrag fuer eine WP-Bindung ohne interaktiven Passwort-Prompt.
 *
 * @param {string} userId - Normalisierte WP-User-ID.
 * @param {string} nsec - Neu generierter nsec.
 * @returns {Promise<object>} Neuer Keyring-Eintrag.
 * @throws {Error} Wenn der Signer vorab interaktiv entsperrt werden muss.
 */
async function createWpBridgeBindingEntry(userId, nsec) {
    const entryName = wpBindingLabelForUser(userId);
    if (sessionPassword) {
        return createKeyringEntry(nsec, sessionPassword, entryName);
    }
    if (hasSessionUnlockMaterial()) {
...
/**
 * Stellt fuer einen WP-User einen gebundenen Signer-Key sicher (laden oder neu erzeugen).
 * Laeuft bewusst ohne blockierenden Passwort-Prompt, um Bridge-Timeouts in eingebetteten iframes zu vermeiden.
 *
 * @param {string} userIdRaw - Rohwert der WP-User-ID aus der Bridge-Anfrage.
 * @returns {Promise<object>} Bindungs- und Schluesselinformationen fuer die Bridge-Antwort.
 */
async function ensureWpUserKey(userIdRaw) {
    ...
    if (boundKeyId) {
        ...
        if (existingIndex >= 0) {
            ...
            if (
                existingEntry.id === activeKeyId &&
                activeUser &&
                typeof activeUser.pubkey === "string" &&
                typeof activeUser.npub === "string"
            ) {
                appendRequestLog(`WP-Binding aktiv: ${userId} -> ${keyDisplayName(existingEntry, existingIndex)}`);
                return {
                    userId,
            ...
            const existingNsec = await decryptKeyringEntryForWpBridge(existingEntry);
    ...
    const newEntry = await createWpBridgeBindingEntry(userId, nsec);
...
/**
 * Baut eine aussagekraeftige Fehlermeldung, wenn WebCrypto nicht verfuegbar ist.
 * Hintergrund: `crypto.subtle` ist im Browser nur in Secure Contexts verfuegbar.
 *
 * @returns {string} Nutzerorientierte Fehlermeldung mit lokalen Workaround-Hinweisen.
 */
function getWebCryptoUnavailableMessage() {
    const protocol = String(window.location.protocol || "");
    const hostname = String(window.location.hostname || "");
    const isSecure = !!window.isSecureContext;
    const hostHint = hostname ? `Host: ${hostname}` : "Host unbekannt";

    if (protocol === "http:" && hostHint && hostHint.includes(".test")) {
        return `${hostHint}. WebCrypto (crypto.subtle) ist im Browser nur in Secure Contexts verfuegbar. ` +
            "Nutze HTTPS (z. B. mkcert) oder localhost. " +
            "Temporärer Chromium-Workaround: --unsafely-treat-insecure-origin-as-secure=http://forums.test";
    }
    ...
    return `${hostHint}. WebCrypto (crypto.subtle) ist nicht verfuegbar. ` +
        "Pruefe Browser-Richtlinien, HTTPS-Setup und Extensions/Security-Policies.";
}
...
async function startSigner() {
    ...
    // Periodischer Verbindungs-Refresh (Heartbeat) um Disconnects im Desktop-Modus zu verhindern
    setInterval(async () => {
        devLog("Heartbeat: Überprüfe Relay-Verbindungen...");
        try {
            // NDK verbindet getrennte Relays neu
            await ndk.connect();
            // Stelle sicher, dass das RPC Backend auch verbunden bleibt
            await ensureBackendRpcRelaysConnected(nip46Backend, 5000, 1);
        } catch (err) {
            devLog("Heartbeat Fehler:", err);
        }
